...
def is_ninja_available():
    ...
    try:
        if shutil.which("ninja"):
            print("shutil.which('ninja')===", shutil.which("ninja"))
            subprocess.check_output(["ninja", "--version"])
            return True
        
        # Manually check in the Conda environment
        username = os.getlogin()
        conda_ninja_path = f"/home/{username}/.conda/envs/moellava_amd/bin/ninja"
        if os.path.exists(conda_ninja_path):
            subprocess.check_output([conda_ninja_path, "--version"])
            return True
