    ...
    def _create_param_groups_fp16_flat_cpu_memory(self):
        ...
        for j, param_group in enumerate(self.trainable_param_groups):
            ...
            if flat_buffer_size > 0:
                ...
                buf = torch.empty(int(flat_buffer_size), dtype=self.dtype)
                if self.offload_param_pin_memory:          # <- 新增判斷
                    buf = get_accelerator().pin_memory(buf)
                self.param_groups_fp16_flat_cpu_memory.append(buf)
            else:
