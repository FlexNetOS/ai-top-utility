...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if image_tower is not None:
            print("if image_tower is not None=====================")
            if self.get_image_tower() is None:
                print("self.get_image_tower() is None=============")
                image_tower = build_image_tower(model_args)
                print("image_tower======111", image_tower)
                if fsdp is not None and len(fsdp) > 0:
                    print("if fsdp is not None and len(fsdp) > 0:")
                ...
                if fsdp is not None and len(fsdp) > 0:
                    ...
                    print("else image_tower============", image_tower)
        ...
        if image_tower is not None and video_tower is not None:  # TODO: support different hidden_size
            print("image_tower.hidden_size==================================", image_tower.hidden_size)
            print("video_tower.hidden_size==================================", video_tower.hidden_size)
